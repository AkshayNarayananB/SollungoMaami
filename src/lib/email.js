...
// Use process.env for Node.js backend environments
const adminEmail = process.env.GMAIL_USER;
const adminPass = process.env.GMAIL_PASS;
 ...
 * Can be called by Cron jobs or API routes.
 */
export const sendNotification = async (data) => {
  ...
  // SCENARIO 1: Mail to Admin (New Comment)
  if (type === 'new_comment') {
    mailOptions = {
  ...
  // SCENARIO 2: Mail to Guest (Reply from Admin)
    ...
    // If invalid type or missing email
    console.warn("Skipping email: Invalid type or missing recipient");
    return;
  }
  ...
  try {
    ...
    console.log(`✅ Email sent to ${mailOptions.to} [Type: ${type}]`);
  } catch (error) {
